...
// ─────────────────────────────────────────────────────────────────────
// Business Central widget elements.
//
// Every tile binds `$state` to the tool's parent path
// (`/business-central/<tool>`) and pushes it through `rows()` first,
// because the hosted BC MCP server's response envelope is not publicly
// documented (preview): it may be a bare array, an OData `{ value: [...] }`
// object, or a JSON string inside MCP text content. All of that tolerance
// lives here so the widget JSON stays dumb.
// ─────────────────────────────────────────────────────────────────────
...
function parseRows(input: unknown, depth = 0): Row[] {
  if (input == null || depth > 4) return [];
  if (Array.isArray(input)) {
    return input.filter((r): r is Row => r != null && typeof r === 'object');
  }
  ...
  if (typeof input === 'object') {
    ...
    if (Array.isArray(obj.content)) {
      for (const part of obj.content as Row[]) {
...
function money(value: unknown, rowCurrency: unknown, fallback: unknown): string {
  const n = toNumber(value);
  const code =
    (typeof rowCurrency === 'string' && rowCurrency.trim()) ||
...
function ddMmmYy(d: Date): string {
...
// Args: { value: unknown }
const rows: ComputedFunction = (args) => parseRows(args.value);
...
const ar_view: ComputedFunction = (args) => {
  const data = parseRows(args.value);
  const currency = args.currency;
  const topLimit = typeof args.topLimit === 'number' ? (args.topLimit as number) : 5;

  let total = 0;
  let current = 0;
  const periods = [0, 0, 0];
  const perCustomer: Array<{ name: string; number: string; overdue: number; balance: number; code: unknown }> = [];
...
const invoices_view: ComputedFunction = (args) => {
  const data = parseRows(args.value);
  const currency = args.currency;
...
const stock_view: ComputedFunction = (args) => {
  const data = parseRows(args.value);
  const currency = args.currency;
  const threshold = typeof args.threshold === 'number' ? (args.threshold as number) : 10;
  const limit = typeof args.limit === 'number' ? (args.limit as number) : 8;

  const stocked = data.filter((r) => {
    const type = String(r.type ?? '').toLowerCase();
    if (type === 'service' || type.startsWith('non')) return false;
    return !r.blocked;
  });
